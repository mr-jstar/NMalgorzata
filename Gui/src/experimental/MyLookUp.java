 ...
 * one, in which case the same array is applied to all bands, or it must equal
 * the number of Source Raster bands.
 * <p>
 * For BufferedImages, the lookup operates on color and alpha components. The
 * number of lookup arrays may be one, in which case the same array is applied
 * to all color (but not alpha) components. Otherwise, the number of lookup
 * arrays may equal the number of Source color components, in which case no
 * lookup of the alpha component (if present) is performed. If neither of these
 * cases apply, the number of lookup arrays must equal the number of Source
 * color components plus alpha components, in which case lookup is performed for
 * all color and alpha components. This allows non-uniform rescaling of
 * multi-band BufferedImages.
 * <p>
 ...
 * regard to whether the data is premultiplied. If a color conversion is
 * required to the destination ColorModel, the premultiplied state of both
 * source and destination will be taken into account for this step.
 * <p>
 * Images with an IndexColorModel cannot be used.
 * <p>
 * If a RenderingHints object is specified in the constructor, the color
 * rendering hint and the dithering hint may be used when color conversion is
 * required.
 * <p>
 ...
 */
public class MyLookUp implements BufferedImageOp, RasterOp {
    ...
    /**
     * Constructs a <code>LookupOp</code> object given the lookup table and a
     * <code>RenderingHints</code> object, which might be <code>null</code>.
     *
     * @param lookup the specified <code>LookupTable</code>
     * @param hints the specified <code>RenderingHints</code>, or
     * <code>null</code>
     */
    public MyLookUp(LookupTable lookup, RenderingHints hints) {
        this.ltable = lookup;
        this.hints = hints;
     ...
     *
     * @return the <code>LookupTable</code> of this <code>LookupOp</code>.
     */
    ...
    /**
     * Performs a lookup operation on a <code>Raster</code>. If the destination
     * <code>Raster</code> is <code>null</code>, a new <code>Raster</code> will
     * be created. The <code>IllegalArgumentException</code> might be thrown if
     * the source <code>Raster</code> and the destination <code>Raster</code> do
     * not have the same number of bands or if the number of arrays in the
     * <code>LookupTable</code> does not meet the restrictions stated in the
     * class comment above.
     *
     * @param src the source <code>Raster</code> to filter
     * @param dst the destination <code>WritableRaster</code> for the filtered
     * <code>src</code>
     * @return the filtered <code>WritableRaster</code>.
     * @throws IllegalArgumentException if the source and destinations rasters
     * do not have the same number of bands, or the number of arrays in the
     * <code>LookupTable</code> does not meet the restrictions described in the
     * class comments.
     *
     */
    public final WritableRaster filter(Raster src, WritableRaster dst) {
        ...
        if (dst == null) {
            ...
            throw new IllegalArgumentException("Width or height of Rasters do not "
                    + "match");
        }
        dstLength = dst.getNumBands();

        if (numBands != dstLength) {
            throw new IllegalArgumentException("Number of channels in the src ("
                    + numBands
    ...
    /**
     * Returns the bounding box of the filtered destination image. Since this is
     * not a geometric operation, the bounding box does not change.
     *
    ...
    /**
     * Returns the bounding box of the filtered destination Raster. Since this
     * is not a geometric operation, the bounding box does not change.
     *
     ...
     * @param destCM the destination's <code>ColorModel</code>, which can be
     * <code>null</code>.
     ...
     * and number of bands, given this source.
     *
     ...
     * source. If <code>dstPt</code> is not <code>null</code>, it will be used
     * to hold the return value. Since this is not a geometric operation, the
     * <code>srcPt</code> will equal the <code>dstPt</code>.
     *
     * @param srcPt a <code>Point2D</code> that represents a point in the source
     * image
     * @param dstPt a <code>Point2D</code>that represents the location in the
     * destination
    ...
    private final void shortFilter(ShortLookupTable lookup, Raster src,
        ...
        int y = 0;
        int index;
        int maxShort = Short.MAX_VALUE; // (1 << 16) - 1;
        // Loop through the data
        for (y = 0; y < height; y++) {
            tidx = 0;
            for (band = 0; band < numBands; band++, tidx += step) {
